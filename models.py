...
class FeSVBiS(nn.Module): 
    ...
    def __init__(
            self, ViT_name, num_classes,
            num_clients=6, in_channels=3, ViT_pretrained=False,
            initial_block=1, final_block=12, resnet_dropout=None, DP=False, mean=None, std=None
    ) -> None:
        super().__init__()
        ...
        self.std = std
        self.epoch=0
        self.intermediate_features = [None] * num_clients  # Initialize self.intermediate_features here

    def forward(self, x, chosen_block, client_idx):
        # Store intermediate features for each block in the first epoch
        if self.training and self.epoch == 0:
            self.intermediate_features[client_idx] = []

        x = self.resnet50_clients[client_idx](x)
        if self.DP:
            # noise = torch.randn(size=x.shape).cuda() * self.std + self.mean
            noise = torch.randn(size=x.shape) * self.std + self.mean
            x = x + noise

        for block_num in range(chosen_block):
            x = self.vit.blocks[block_num](x)
            # Store intermediate feature if it's the first epoch
            if self.training and self.epoch == 0:
                self.intermediate_features[client_idx].append(
                    (block_num, x.detach().cpu())
                )
...
class SplitFeSViBS(SplitNetwork):
    ...
    def __init__(
        ...
        self.base_dir = base_dir
        self.train_chosen_blocks = [0] * num_clients
        self.intermediate_features = [[] for _ in range(num_clients)]
        self.epoch = 0
    ...
    def train_round(self, client_i):
        ...
        if self.epoch == 0:
            for client_idx in range(self.num_clients):
                for block_num, feature in self.intermediate_features[client_idx]:
                    # Assuming 'imgs' is your original image matrix
                    wasserstein_distance_values = []
                    for img in imgs:
                        wasserstein_distance_values.append(
                            wasserstein_distance(
                                img.numpy().flatten(),
                                feature[client_idx].numpy().flatten()
                            )
                        )
                    # Store the Wasserstein distances for analysis
                    self.wasserstein_distances[client_idx][block_num] = wasserstein_distance_values
